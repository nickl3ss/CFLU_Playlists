...
| 21 | Origin gate on all state-changing routes; state-changing endpoints are POST only | A `POST` with `Content-Type: text/plain` is a CORS *simple request* — the browser sends it without a preflight, so any web page open in the user's browser could call `/api/spotify/call` (with the user's full Spotify scopes), `/api/upload-csv` or `/api/lastfm/sync`; and `GET /api/spotify/logout` was a state-changing GET, triggerable by a plain `<img src>`. `do_POST` now rejects every request whose `Origin` header is not exactly `http://127.0.0.1:{PORT}` or `http://localhost:{PORT}` with `403 {"error": "origin not allowed"}` *before* routing (`_origin_allowed` — exact string compare, no `startswith`, so `http://127.0.0.1:8888.evil.example` cannot slip past), and logout moved from `do_GET` to `do_POST` (GET now 404s like any unknown path; `spotifyLogout()` sends `{ method: 'POST' }`). A *missing* Origin is deliberately allowed: browsers always attach `Origin` to POST requests, so only non-browser clients (curl, scripts, CI) omit it — they are not a CSRF vector, and rejecting them would break local tooling for no security gain. The 127.0.0.1 bind and CSP stay as the outer layers; this closes the remaining in-browser cross-site hole. `/api/spotify/login` + `/callback` stay GET redirects (Invariant 1 unchanged); no token leaves the server (Invariant 2 unchanged). | 2026-09-02 |
